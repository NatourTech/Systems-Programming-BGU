...
Event::Event(std::string team_a_name, std::string team_b_name, std::string name, int time,
    ...
    : team_a_name(team_a_name), team_b_name(team_b_name), name(name),
      ...
      team_b_updates(team_b_updates), description(discription),beforeHalf(true)
{
}
Event::Event():team_a_name(""), team_b_name(""), name(""), time(0), game_updates(), team_a_updates(), team_b_updates(), description(""),beforeHalf(true){}
...
std::string Event::toString(std::string user)
{
std::ostringstream builder;
builder<<"user: " << user<<"\n";
builder<<"team a: " << get_team_a_name()<<"\n";
builder<<"team b: " << get_team_b_name()<<"\n";
builder<<"event name: "<<get_name()<<"\n";
builder<<"time: "<<get_time()<<"\n";
builder<<"general game updates:"<<"\n";
std::map<std::string,std::string> ups = get_game_updates();
for (const auto &pair : ups)
{
    builder<<"\t"<<pair.first<<": "<<pair.second<<"\n";
}
builder<<"team a updates:"<<"\n";
std::map<std::string,std::string> ups_a = get_team_a_updates();
for (const auto &pair : ups_a) 
{
    builder<<"\t"<< pair.first << ": " << pair.second <<"\n";
}
builder<<"team b updates:"<<"\n";
std::map<std::string,std::string> ups_b = get_team_b_updates();
for (const auto &pair : ups_b) 
{
    builder<<"\t"<< pair.first << ": " << pair.second <<"\n";
}
builder<<"description:"<<"\n";
builder<<get_discription()<<"\n";
std::string event = builder.str();
return event;
}

Event::Event(const std::string &frame_body) : team_a_name(""), team_b_name(""), name(""), time(0), game_updates(), team_a_updates(), team_b_updates(), description(""),beforeHalf(true)
{
}
...
names_and_events parseEventsFile(std::string json_path)
{
 ...
 void Event::update_time(int ntime)
 {
     this->time = ntime;
 }