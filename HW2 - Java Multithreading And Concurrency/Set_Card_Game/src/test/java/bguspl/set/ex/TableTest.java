...
class TableTest {
    ...
    void setUp() {
        ...
        Env env = new Env(logger, config, ui, new MockUtil());
    ...
    @Test
    void removeCard_illegal(){
        int slot = 0;
        table.removeCard(slot);
        //shouldnt be called because the slot to card is null at the moment
        verify(ui,never()).removeCard(eq(slot));
    }

    @Test
    void removeCard_legal(){
        fillSomeSlots();
        int slot = 1;
        table.removeCard(slot);
        verify(ui).removeCard(eq(slot));
        int slot2 = 2;
        table.removeCard(slot2);
        verify(ui).removeCard(eq(slot2));
        int invalidSlot = 3;
        table.removeCard(invalidSlot);
        verify(ui,never()).removeCard(eq(invalidSlot));
    }
